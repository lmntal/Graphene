...
class View(var rect: Rect, var color: Color) {
  ...
  var sx = 0.0 // speed x 速度
  var sy = 0.0 // speed y 速度
  var dx = 0.0 // delta x 変位
  var dy = 0.0 // delta y 変位
  var fixed = false
  var selected = false
  var didAppear = false
  ...
  def move() {
    ...
    // TODO: Magic number
    if (abs < 100) {
      rect = Rect(Point(rect.point.x + dx, rect.point.y + dy), rect.dim) // 場所更新
    } else {
...
class Node private(val graph: Graph, val parent: Node, val id: ID, var name: String, var attr: Attr) {
  ...
  private[model] def addEdge(e: Edge) = {
    edges += e;
    neighborNodes += e.adjacentNodeOf(this)
  }

  private[model] def removeEdge(e: Edge) = {
    edges -= e;
    neighborNodes -= e.adjacentNodeOf(this)
  }
